 ...
 * 商家信息
 */
class BusinController extends Controller {
/**
	 * 禁止非空登录
	 */
     ...
     * 商家店铺页面
     */
    public function busin_info(){
    
    	$info = M('Busin_info') -> alias('binfo')
    	-> join('join __STAFF_INFO__ sta on sta.busin_id = binfo.id')
    	-> join('join __BUSIN_TYPE__ btype on binfo.type_id = btype.id')
    	-> where(array('sta.username'=>session('username')))
    	-> field('binfo.name,binfo.people,binfo.phone,binfo.floor,binfo.describe,btype.type_name')
    	-> find();
    	$this ->assign("info",$info);
    	$this -> display();
    }
    ...
    public function active(){
    	...
    	$busin_id = M("staff_info") 
    	-> where(array("username"=>session('username')))
    	-> getField('busin_id');
    	$info = M('staff_info') -> alias('sta')
    	-> join("join __BUSIN_INFO__ binfo on binfo.id = sta.busin_id")
    	-> join("join __BUSIN_ACTIVE__ bactv on bactv.busin_id = binfo.id")
    	-> where(array('bactv.busin_id'=>$busin_id))
    	-> field('binfo.name,bactv.title,bactv.create_time,bactv.content')
    	-> select();
    	$this -> assign("info",$info);
    	$this ->display();
    }
